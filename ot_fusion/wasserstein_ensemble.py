...
# from . import utils # it's only for 'act' mode, no need for 'wgt' mode
...
def get_wassersteinized_layers_modularized(args, networks, activations=None, eps=1e-7, test_loader=None):
    ...
    for idx, ((layer0_name, fc_layer0_weight), (layer1_name, fc_layer1_weight)) in \
        ...
        ## process the bias for fcnet
        if args.model_name == "fcnet":
            # concatenate the bias to weight
            if idx % 2 == 0:
                # continue for the bias layer
                pre_layer0_name = layer0_name
                pre_fc_layer0_weight = fc_layer0_weight
                pre_layer1_name = layer1_name
                pre_fc_layer1_weight = fc_layer1_weight
                pre_idx = idx
                continue
            else:
                layer0_name = pre_layer0_name
                layer1_name = pre_layer1_name
                fc_layer0_weight = torch.cat((pre_fc_layer0_weight, fc_layer0_weight.view(fc_layer0_weight.shape[0], 1)), dim=1)
                fc_layer1_weight = torch.cat((pre_fc_layer1_weight, fc_layer1_weight.view(fc_layer1_weight.shape[0], 1)), dim=1)
                idx = pre_idx
        ...
        #     # for the bias layer
        #     avg_aligned_layers.append((fc_layer0_weight.data + fc_layer1_weight.data)/2)
        #     continue
        else:
            is_conv = False
        ...
        if args.model_name == "fcnet":
            avg_aligned_layers.append(geometric_fc[:, :-1])
            avg_aligned_layers.append(geometric_fc[:,-1].view(-1))
        else:
...
def get_network_from_param_list(args, param_list, test_loader):
    ...
    for key, value in model_state_dict.items():
        ...
        print('param shape:', param_list[layer_idx].shape)
        layer_idx += 1
