...
from combine_nets import compute_skip_matching_multilayer
import pdb
	...
	for trial in range(args.trials):
		...
		if ("pdm" in args.experiment) or ("all" in args.experiment):
			...
			best_sigma0, best_sigma, best_gamma, best_test_acc, best_train_acc, best_weights, res = compute_pdm_matching_multilayer(
				nets_list, train_dl, test_dl, traindata_cls_counts, args.net_config[-1], it=5, sigma=1, sigma0=1, gamma=1,
				device=device
			)
		...
		if ("skip" in args.experiment) or ("all" in args.experiment):
			print("Computing hungarian skip matching")
			best_sigma0, best_sigma, best_gamma, best_test_acc, best_train_acc, best_weights, res = compute_skip_matching_multilayer(
				nets_list, train_dl, test_dl, traindata_cls_counts, args.net_config[-1], it=5, sigma=1, sigma0=1, gamma=1,
				device=device
			)

			logging.debug("****** PDM skip matching ******** ")
			logging.debug("Best Sigma0: %s. Best sigma: %s Best gamma: %s. Best Test accuracy: %s. Train acc: %s. \n"
						  % (str(best_sigma0), str(best_sigma), str(best_gamma), str(best_test_acc), str(best_train_acc)))
		...
		if ("pdm_KL_only" in args.experiment) or ("all" in args.experiment):
			print("Computing hungarian matching")
			best_sigma0, best_sigma, best_gamma, best_test_acc, best_train_acc, best_weights, res = compute_pdm_matching_multilayer(
				nets_list, train_dl, test_dl, traindata_cls_counts, args.net_config[-1], it=5, sigma=1, sigma0=1, gamma=1,
				device=device, KL=3
			...
			logging.debug("****** PDM_KL_only matching ******** ")
			logging.debug("Best Sigma0: %s. Best sigma: %s Best gamma: %s. Best Test accuracy: %s. Train acc: %s. \n"
						  % (str(best_sigma0), str(best_sigma), str(best_gamma), str(best_test_acc), str(best_train_acc)))

			logging.debug("PDM_KL_only log: %s " % str(res))

		if ("pdm_KL2_only" in args.experiment) or ("all" in args.experiment):
			print("Computing hungarian matching")
			best_sigma0, best_sigma, best_gamma, best_test_acc, best_train_acc, best_weights, res = compute_pdm_matching_multilayer(
				nets_list, train_dl, test_dl, traindata_cls_counts, args.net_config[-1], it=5, sigma=1, sigma0=1, gamma=1,
				device=device, KL=4
			...
			logging.debug("****** PDM_KL2_only matching ******** ")
			logging.debug("Best Sigma0: %s. Best sigma: %s Best gamma: %s. Best Test accuracy: %s. Train acc: %s. \n"
						  % (str(best_sigma0), str(best_sigma), str(best_gamma), str(best_test_acc), str(best_train_acc)))

			logging.debug("PDM_KL2_only log: %s " % str(res))
			...
			for (sigma0, sigma, gamma) in product(sigma0s, sigmas, gammas):
				...
				for i, comm_round in enumerate(range(args.communication_rounds)):
					...
					conf_matrix_train, conf_matrix_test = compute_iterative_pdm_matching(
						...
						sigma, sigma0, gamma, it, old_assignment=assignment, device=device, KL_reg=0.01
					)
