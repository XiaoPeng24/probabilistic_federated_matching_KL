...
def get_parser():
    ...
    parser.add_argument('--train_online_altmin', type=bool, default=False, metavar='T',
                        help='train the model in online alternative minimization way')

    # PDM matching settings
    parser.add_argument('--iter_epochs', type=int, required=False, default=15, help="Epochs for PDM-iterative method")
...
def train_net(net_id, net, train_dataloader, test_dataloader, args, reg_base_weights=None,
    ...
    if args.model == "fcnet" and args.model == "ffnet":
    ...
    if args.train_online_altmin:
        ...
        net = get_mods(net, optimizer='Adam', optimizer_params={'lr': args.lr_weights},
                       scheduler=lambda epoch: 1 / 2 ** (epoch // args.lr_half_epochs))
        net[-1].optimizer.param_groups[0]['lr'] = args.lr_out
        mu = args.mu
        mu_max = 10 * args.mu
    ...
    for epoch in range(args.epochs):
        for batch_idx, (x, target) in enumerate(train_dataloader):
            ...
            if args.train_online_altmin:
                with torch.no_grad():
                    outputs, codes = get_codes(net, x)

                # (2) Update codes
                codes = update_codes(codes, net, target, criterion, mu, lambda_c=args.lambda_c,
...
def local_train(nets, args, net_dataidx_map, device="cpu"):
    ...
    for net_id, net in nets.items():
        ...
        if args.retrain:
            train_acc, test_acc = train_net(net_id, net, train_dl_local, test_dl_local, args, save_path=None,
                                            device=device)
...
def fedavg_comm(batch_weights, model_meta_data, layer_type, net_dataidx_map,
    ...
    for cr in range(comm_round):
        ...
        for worker_index in range(args.n_nets):
            ...
            retrained_cnn = local_retrain_fedavg(len(dataidxs), (train_dl_local, test_dl_local),
                                                 batch_weights[worker_index], args,
                                                 device=device)
...
def fedprox_comm(batch_weights, model_meta_data, layer_type, net_dataidx_map,
    ...
    for cr in range(comm_round):
        ...
        for worker_index in range(args.n_nets):
            ...
            retrained_cnn = local_retrain_fedprox(len(dataidxs), (train_dl_local, test_dl_local),
                                                  batch_weights[worker_index], mu=0.001,
...
def run_exp(n):
    ...
    if args.train_online_altmin:
        log_dir = os.path.join(log_dir, "online")
    else:
        log_dir = os.path.join(log_dir, "offline")
    ...
    for trial in range(args.trials):
        ...
        if ("FedAvg" in args.experiment) or ("all" in args.experiment):
            ...
            avg_train_acc, avg_test_acc, _, _ = compute_fedavg_accuracy(nets_list, train_dl, test_dl,
                                                                        traindata_cls_counts, n_classes, device=device)
        ...
        if ("pdm_KL" in args.experiment) or ("all" in args.experiment):
            ...
            for KL_reg in KL_regs:
                ...
                if args.model == "fcnet" or args.model == "ffnet":
                ...
                trials_res[trial]["pdm_KL_reg " + str(KL_reg)]["Best Result"] = {"Best Sigma0": res['sigma0'],
        ...
        if ("pdm_I" in args.experiment) or ("all" in args.experiment):
            ...
            for I_reg in I_regs:
                ...
                logger.info("Best Sigma0: %s. Best sigma: %s Best gamma: %s. Best Test accuracy: %s. Train acc: %s. \n"
                            % (str(best_sigma0), str(best_sigma), str(best_gamma), str(best_test_acc),
                               str(best_train_acc)))

                logger.info("PDM_I log: %s " % str(res))

                trials_res[trial]["pdm_I_reg " + str(I_reg)]["Best Result"] = {"Best Sigma0": best_sigma0,
        ...
        if ("pdm_neg_I" in args.experiment) or ("all" in args.experiment):
            ...
            for I_reg in I_regs:
                ...
                trials_res[trial]["pdm_neg_I_reg " + str(I_reg)]["Best Result"] = {"Best Sigma0": best_sigma0,
                                                                                   "Best sigma": best_sigma,
                                                                                   "Best gamma": best_gamma,
                                                                                   "Best Test accuracy": best_test_acc,
                                                                                   "Train acc": best_train_acc}
        ...
        if ("pdm_coff" in args.experiment) or ("all" in args.experiment):
            ...
            for coff in coffs:
                ...
                trials_res[trial]["pdm_coff 1-" + str(coff)]["Best Result"] = {"Best Sigma0": best_sigma0,
                                                                               "Best sigma": best_sigma,
                                                                               "Best gamma": best_gamma,
                                                                               "Best Test accuracy": best_test_acc,
                                                                               "Train acc": best_train_acc}
        ...
        if ("pdm_fix" in args.experiment) or ("all" in args.experiment):
            ...
            for coff in coffs:
                ...
                trials_res[trial]["pdm_fix " + str(coff)]["Best Result"] = {"Best Sigma0": best_sigma0,
                                                                            "Best sigma": best_sigma,
                                                                            "Best gamma": best_gamma,
                                                                            "Best Test accuracy": best_test_acc,
                                                                            "Train acc": best_train_acc}
        ...
        if ("pdm_iterative" in args.experiment) or ("all" in args.experiment):
            ...
            for KL_reg in KL_regs:
                logger.info("Parameter setting: sigma0 = %f, sigma = %f, gamma = %f" % (
                args.pdm_sig0, args.pdm_sig, args.pdm_gamma))
                ...
                for i, comm_round in enumerate(range(args.communication_rounds)):
                    ...
                    logger.info("Communication: %d, Train acc: %f, Test acc: %f, Shapes: %s" % (
                    ...
                    for net_id, net in iter_nets.items():
                        ...
                        train_net(net_id, net, net_train_dl, net_test_dl, expepochs, lr_iter, reg_iter,
                                  net_weights_new[net_id],
                                  device=device)
...
def abli_exp(n=[10], dataset="mnist"):
    ...
    for partition in partitions:
        ...
        logdir = os.path.join('online_altmin_exp',
