...
def get_parser():
    ...
    parser.add_argument('--device', type=int, required=False, default=1.0, help="Device to run")
        ...
        for batch_idx, (x, target) in enumerate(train_dataloader):
            ...
            if reg_base_weights is None:
                ...
                for pname, param in net.named_parameters():
                    ...
                    l2_reg = l2_reg + 0.5 * torch.pow((param - torch.from_numpy(ref_param).to(device).float()), 2).sum()
            #pdb.set_trace()
...
def run_exp(KL_reg):
    print("Current KL_reg  is %f " % KL_reg)
    #args.n_nets = n_nets
    #args.logdir = os.path.join(logdir, "n_nets "+str(n_nets))
    #gpu_id = int(n_layer+2 % 4)
    #gpu_id = int(KL_reg * 1456) % 4 
    #gpu_id = 3 if int(KL_reg * 1456)%2 == 0 else 2
    gpu_id = args.device
    #gpu_id = 0
    ...
    log_dir = os.path.join(args.logdir, "KL_reg "+str(KL_reg))
    ...
    for trial in range(args.trials):
        ...
        #lr_iter = args.lr
        #reg_iter = args.reg
        expepochs = args.epochs
        assignment = None

        for i, comm_round in enumerate(range(args.communication_rounds)):

            print("Executing Communication: %d" % comm_round)
            logging.debug("Executing Communication: %d" % comm_round)

            trials_res[trial]["comm "+str(comm_round)] = {}
            ...
            for net_id, net in nets.items():
                ...
                trainacc, testacc = train_net(net_id, net, net_train_dl, net_train_dl, expepochs, args.lr, args.reg, device=device)
                # a huge mistake above, mistakely writing net_test_dl as net_train_dl
                if comm_round == 0:
                    iter_nets[net_id] = copy.deepcopy(net)
                else:
                    train_net(net_id, iter_nets[net_id], net_train_dl, net_test_dl, expepochs, args.lr, args.reg, net_weights_new[net_id],
                              device=device)

                local_train_accs.append(trainacc)
                local_test_accs.append(testacc)

            expepochs = args.iter_epochs

            end = datetime.datetime.now()
            timing = (end - start).seconds
            trials_res[trial]["comm "+str(comm_round)]['training timing'] = timing

            trials_res[trial]["comm "+str(comm_round)]['local_train_accs'] = local_train_accs
            trials_res[trial]["comm "+str(comm_round)]['local_test_accs'] = local_test_accs
            ...
            if ("u-ensemble" in args.experiment) or ("all" in args.experiment):
                ...
                trials_res[trial]["comm "+str(comm_round)]["Uniform ensemble (Train acc)"] = uens_train_acc
                trials_res[trial]["comm "+str(comm_round)]["Uniform ensemble (Test acc)"] = uens_test_acc

            if ("FedAvg" in args.experiment):
                ...
                trials_res[trial]["comm "+str(comm_round)]["FedAvg (Train acc)"] = avg_train_acc
                trials_res[trial]["comm "+str(comm_round)]["FedAvg (Test acc)"] = avg_test_acc
            ...
            if ("pdm_KL" in args.experiment) or ("all" in args.experiment):
                ...
                conf_matrix_train, conf_matrix_test = compute_iterative_pdm_matching(
                    ...
                    args.pdm_sig, args.pdm_sig0, args.pdm_gamma, it=5, old_assignment=assignment, device=device, KL_reg=KL_reg
                )
                #best_sigma0, best_sigma, best_gamma, test_acc, train_acc, best_weights, res = compute_pdm_matching_multilayer(
                #    iter_nets_list, train_dl, test_dl, traindata_cls_counts, args.net_config[-1], it=5, sigma=args.pdm_sig, 
                #    sigma0=args.pdm_sig0, gamma=args.pdm_gamma, device=device, KL_reg=KL_reg
                #)
                trials_res[trial]["comm "+str(comm_round)]["pdm_KL (Train acc)"] = train_acc
                trials_res[trial]["comm "+str(comm_round)]["pdm_KL (Test acc)"] = test_acc

                iter_nets = load_new_state(iter_nets, net_weights_new)

            args.lr *= args.lr_decay
            args.reg *= args.reg_fac
...
def abli_exp(n=10, dataset="mnist", partition="hetero-dir"):
    ...
    #for partition in partitions:
        ...
        #                      dataset, partition)
    logdir = os.path.join('KL_regs_iter',
                            dataset, partition)
    ...
    with contextlib.closing(Pool(args.num_pool_workers)) as po:
        pool_results = po.map_async(run_exp, (KL_reg for KL_reg in KL_regs))
        results_list = pool_results.get()

    for KL_reg, result in zip(KL_regs, results_list):
        abli_res[KL_reg] = result
...
if __name__ == "__main__":
    ...
    abli_exp(dataset=args.dataset, partition=args.partition)
