...
def get_parser():
    ...
    parser.add_argument('--use_freq', type=bool, required=False, default=False, help='whether use the freq info in the output bias')
    ...
    for trial in range(args.trials):
        ...
        if ("pdm_KL" in args.experiment) or ("all" in args.experiment):
            ...
            for KL_reg in KL_regs:
                ...
                trials_res[trial]["pdm_KL_reg "+str(KL_reg)]["Best Result"] = {"Best Sigma0": best_sigma0, "Best sigma": best_sigma, 
        ...
        if ("skip" in args.experiment) or ("all" in args.experiment):
            ...
            trials_res[trial]["skip_pdm"]["Best Result"] = {"Best Sigma0": best_sigma0, "Best sigma": best_sigma, 
                                                         "Best gamma": best_gamma, "Best Test accuracy": best_test_acc,
                                                         "Train acc": best_train_acc}
            trials_res[trial]["skip_pdm"]["PDM log"] = str(res)
            trials_res[trial]["skip_pdm"]["timing"] = timing
...
def abli_exp(n=10, dataset="mnist"):
    ...
    for partition in partitions:
        ...
        #                      dataset, partition)
        logdir = os.path.join('skip_pdm_compare',
                              dataset, partition)
