...
def run_exp(n_layer):
    print("Current n_layer is %d " % n_layer)
    #args.n_nets = n_nets
    #args.logdir = os.path.join(logdir, "n_nets "+str(n_nets))
    #gpu_id = int(n_layer+2 % 4)
    gpu_id = 3 if n_layer%2 == 0 else 1
    device_str = "cuda:" + str(gpu_id)
    device = torch.device(device_str if torch.cuda.is_available() else "cpu")
    print("Current device is :", device)

    if args.dataset == "mnist":
        args.net_config = list(map(int, ("784, "+"100, "*n_layer+"10").split(', ')))
    elif args.dataset == "cifar10":
        args.net_config = list(map(int, ("3072, "+"100, "*n_layer+"10").split(', ')))
    if not os.path.exists(os.path.join(args.logdir, "n_layers "+str(n_layer))):
        mkdirs(os.path.join(args.logdir, "n_layers "+str(n_layer)))
    with open(os.path.join(args.logdir, "n_layers "+str(n_layer), 'experiment_arguments.json'), 'w') as f:
    ...
    for trial in range(args.trials):
        ...
        if ("pdm" in args.experiment) or ("all" in args.experiment):
            ...
            trials_res[trial]["pdm_match"]["timing"] = str(timing)                                               
        ...
        if ("pdm_KL" in args.experiment) or ("all" in args.experiment):
            ...
            trials_res[trial]["pdm_KL_match"]["timing"] = timing
        ...
        if ("pdm_KL2" in args.experiment) or ("all" in args.experiment):
            ...
            trials_res[trial]["pdm_KL2_match"]["timing"] = timing
        ...
        with open(os.path.join(args.logdir, "n_layers "+str(n_layer), 'trial'+str(trial)+'.json'), 'w') as f:
            json.dump(trials_res[trial], f) 

    with open(os.path.join(args.logdir, "n_layers "+str(n_layer), 'trials_res.json'), 'w') as f:
...
def abli_exp(n_layers=10, dataset="mnist", partition='hetero-dir'):
    ...
    n_layer_list = [i+4 for i in range(n_layers-3)] # don't need layer_num 1
    ...
    for partition in partitions:
        ...
        #                      dataset, partition)
        logdir = os.path.join('log_abli', '2020-08-17 19',
                              dataset, partition)
        args.logdir = logdir
        ...
        with contextlib.closing(Pool(args.num_pool_workers)) as po:
            pool_results = po.map_async(run_exp, (n_layer for n_layer in n_layer_list))
            results_list = pool_results.get()

        for n_layer, result in zip(n_layer_list, results_list):
            abli_res[n_layer] = result
...
if __name__ == "__main__":
    ...
    abli_exp(n_layers=args.n_layers, dataset=args.dataset, partition=args.partition)
